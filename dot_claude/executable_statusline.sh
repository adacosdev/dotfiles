...
get_mcp_servers() {
  ...
  if [ -f "$MCP_CACHE_FILE" ]; then
    # stat syntax differs: -c %Y on Linux, -f %m on macOS
    if [[ "$(uname)" == "Darwin" ]]; then
      CACHE_AGE=$(($(date +%s) - $(stat -f %m "$MCP_CACHE_FILE" 2>/dev/null || echo 0)))
    else
      CACHE_AGE=$(($(date +%s) - $(stat -c %Y "$MCP_CACHE_FILE" 2>/dev/null || echo 0)))
    fi
  ...
  # Fallback to home dir config if current project has no MCP
  if [ -z "$SERVERS" ]; then
